...
public class RecommendationService {
    private static final int MAX_GAME_SEED = 3;  //如果写太多，API反应时间太久
    ...
    //为了推荐recommendation，需要3个private API(helper functions)去执行这个功能
    // 1. 已经收藏过的就exclude，不要再推荐
    // 2. Twitch给我们提供的API不一样 - Stream可以直接收到很多；但是Clip和Video只能每次拿一个。所以video每个game id要哎个的搜索一次。for loop
    // 3. Recommended Clips跟video一样 - 需要两个for loop
